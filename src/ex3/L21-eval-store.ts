...
import { add, map, reduce, repeat, zipWith } from "ramda";
import {
...
import { applyEnv, makeExtEnv, Env, Store, setStore, extendStore, ExtEnv, applyStore, theGlobalEnv, globalEnvAddBinding, theStore, isExtEnv, getAddress } from "./L21-env-store";
        ...
        isBoolExp(exp) ? makeOk(exp.val) :
            isStrExp(exp) ? makeOk(exp.val) :
                isPrimOp(exp) ? makeOk(exp) :
                    isVarRef(exp) ? bind(applyEnv(env, exp.var), (address: number) => applyStore(theStore, address)) : //here
...
const applyClosure = (proc: Closure, args: Value[]): Result<Value> => {
    ...
    // const addresses: number[] = (isExtEnv(proc.env)) ? proc.env.addresses : proc.env.addresses[0];
    map((val: Value) => extendStore(theStore, val), args);
    const addresses: number[] = map((val: Value) => getAddress(theStore, val), args); // here
...
const evalDefineExps = (def: DefineExp, exps: Exp[]): Result<Value> =>
    bind(applicativeEval(def.val, theGlobalEnv),
        (rhs: Value) => {
            ...
            const address = getAddress(theStore, rhs);
...
const evalLet = (exp: LetExp, env: Env): Result<Value> => {
    ...
    return bind(vals, (vals: Value[]) => {
        ...
        const addresses: number[] = map((val: Value) => getAddress(theStore, val), vals); // here 
